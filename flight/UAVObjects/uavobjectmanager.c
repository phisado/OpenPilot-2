...
int32_t UAVObjSave(UAVObjHandle obj_handle, uint16_t instId)
{
	...
	if (UAVObjIsMetaobject(obj_handle)) {
		...
		if (PIOS_FLASHFS_Compact_ObjSave(obj_handle, instId, (uint8_t*) MetaDataPtr((struct UAVOMeta *)obj_handle)) != 0)
			return -1;
	} else {
		InstanceHandle instEntry = getInstance( (struct UAVOData *)obj_handle, instId);
		...
		if (PIOS_FLASHFS_Compact_ObjSave(obj_handle, instId, InstanceData(instEntry)) != 0)
			return -1;
	}
#endif
#if defined(PIOS_INCLUDE_SDCARD)
